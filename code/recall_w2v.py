...
def word2vec(df_, f1, f2, model_path):
    ...
    print(tmp[:5])
    #sentences is [[30760, 157507], [289197, 63746], [article_id,article_id]点击序列
    sentences = tmp['{}_{}_list'.format(f1, f2)].values.tolist()
    del tmp['{}_{}_list'.format(f1, f2)]

    print(f'sentences is {len(sentences)}')
    words = []
    ...
    #获取每个物品（或文章）的向量表示，并将其存储在 article_vec_map 字典
    # article_vec_map = {}
    ...
    article_vec_map = {}
    for word in set(words):
        if word in model.wv:  # 使用 model.wv 检查词是否存在
            article_vec_map[int(word)] = model.wv[word]  # 获取词向量
